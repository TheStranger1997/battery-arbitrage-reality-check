...
def score_metric(
    ...
    excess = (ideal_low - value) if value < ideal_low else (value - ideal_high)
    decay_width = range_width / tolerance
    return round(100.0 * max(0.0, 1.0 - excess / decay_width), 2)
...
def main() -> None:
    ...
    for metric, (lo, hi, unit, desc) in IDEAL_RANGES.items():
        ...
        records.append({
            ...
            "ideal_low": lo, "ideal_high": hi, "unit": unit,
            "score": sc, "rating": _rating(sc),
    ...
    for r in records:
        print(f"  {r['metric']:30s}  {r['value']:7.2f}  →  {r['score']:5.1f}  [{r['rating']}]")
